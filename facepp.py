...
class API(object):
    ...
    def __init__(self, key, secret, srv=None,
        ...
        _setup_apiobj(self, self, [])# setup呼んでる

    def update_request(self, request):
        """overwrite this function to update the request before sending it to
        server"""
        # この関数を上書きしてリクエストを送る前に変更できるで
        # どう変更したらいいかはわからへん
        #　ここでdata書き換えてオプション付けようとしたけど無理やから下に直接書き足したで
...
def _setup_apiobj(self, api, path):
    ...
    for i in _APIS:
        ...
        if i[:lvl] == path and cur not in done:
            ...
            setattr(self, cur, _APIProxy(api, i[:lvl + 1]))# Proxy呼んでる


# たぶんここで通信してる
class _APIProxy(object):
    _api = None
    """underlying :class:`API` object"""#underlying=根底

    _urlbase = None

    def __init__(self, api, path):
        _setup_apiobj(self, api, path)# setup呼んでる
    ...
    def __call__(self, *args, **kargs):
        ...
        #*　ごり押し detect option 追加
        if 'detect' in url:
            form.add_field("return_landmark", "1")
            form.add_field("return_attributes", "gender,age,smiling,headpose,facequality,blur,eyestatus,emotion,ethnicity,beauty,mouthstatus,eyegaze,skinstatus")
        #*　動いたからこのままいくで
        ...
        while True:
            #retry -= 1
            try:
                ret = urllib2.urlopen(request, timeout=self._api.timeout).read() #たぶんここでPOST通信
                break
